...
model = smp.deeplabv3.model.DeepLabV3('efficientnet-b4', classes=1, activation='sigmoid', encoder_weights=None)
model.to(device)
...
metric_smp = [smp.utils.metrics.IoU(threshold=0.8, activation=None)]
metric_default = smp.utils.metrics.IoU(threshold=0.8, activation=None)
...
for i in range(0, 30):
    ...
    if max_score < valid_logs['iou_score']:
        ...
        torch.save(model.state_dict(), 'smp_trainer_model.pt')
        print('Model saved')
...
#
#     model.train()
#     with tqdm(enumerate(train), postfix={'train_loss': 0.0}) as bar:
#     #print(len(bar))
#         for idx, data in bar:
#             print(len(data))
#             input = data['image']
...
#     del data, target, idx
#     with torch.no_grad():
#         with tqdm(enumerate(valid), postfix={'valid_loss': 0.0, 'valid_IoU': 0.0}) as bar:
#             for idx, data in bar:
#                 input = data['image']
#                 target = data['mask']
#
...
#                 score = metric_default(target, output)
...
#             valid_loss_min,
#             valid_loss))
#         torch.save(model.state_dict(), 'default_trainer_model.pt')
